import { Platform } from '../entities/Platform.js';
import { CrumblingPlatform } from '../entities/CrumblingPlatform.js';

export class World2Level3 {
    constructor() {
        this.name = "World 2-3";
        this.world = 2;
        this.start = { x: 50, y: 600, width: 80, height: 100 };
        this.goal = { x: 600, y: 150, width: 80, height: 100 };

        this.threeStarTime = 10;
        this.twoStarTime = 15;

        this.platforms = [
            new Platform(0, 650, 200, 50),
            new Platform(250, 600, 120, 20),
            new Platform(450, 550, 120, 20),
            new Platform(300, 450, 120, 20),
            new Platform(500, 350, 120, 20),
            new Platform(350, 250, 120, 20),
            new Platform(200, 640, 80, 20),
            new Platform(420, 600, 80, 20),
            new Platform(340, 120, 20, 100),
        ];

        this.gluePlatforms = [
        ];

        this.crumblingPlatforms = [
            new CrumblingPlatform(100, 360, 120, 20),
            new CrumblingPlatform(320, 300, 20, 80),
            new CrumblingPlatform(120, 180, 20, 100),
            new CrumblingPlatform(620, 600, 100, 20),
        ];

        this.blinkingPlatforms = [
        ];

        this.spikes = [
            { x: 220, y: 620, width: 30, height: 30 },
            { x: 420, y: 570, width: 30, height: 30 },
            { x: 540, y: 320, width: 40, height: 40 },
            { x: 520, y: 520, width: 40, height: 40 },
            { x: 460, y: 520, width: 40, height: 40 },
            { x: 380, y: 220, width: 40, height: 40 },
            { x: 140, y: 320, width: 40, height: 40 },
            { x: 620, y: 560, width: 40, height: 40 },
            { x: 420, y: 220, width: 40, height: 40 },
        ];

        this.blackHoles = [
            {
                x: 380,
                y: 500,
                width: 60,
                height: 60,
                pullRadius: 400,
                pullStrength: 850,
                killRadius: 40
            },
            {
                x: 450,
                y: 300,
                width: 60,
                height: 60,
                pullRadius: 400,
                pullStrength: 850,
                killRadius: 40
            },
            {
                x: 640,
                y: 620,
                width: 60,
                height: 60,
                pullRadius: 350,
                pullStrength: 800,
                killRadius: 40
            },
            {
                x: 60,
                y: 200,
                width: 60,
                height: 60,
                pullRadius: 350,
                pullStrength: 800,
                killRadius: 40
            },
            {
                x: 400,
                y: 160,
                width: 60,
                height: 60,
                pullRadius: 350,
                pullStrength: 800,
                killRadius: 40
            },
        ];

        this.turrets = [
        ];

        this.movingSpikes = [
            {
                x: 160,
                y: 80,
                width: 20,
                height: 20,
                startX: 160,
                endX: 360,
                speed: 150,
                direction: 1
            },
            {
                x: 480,
                y: 460,
                width: 40,
                height: 40,
                startX: 480,
                endX: 680,
                speed: 150,
                direction: 1
            },
            {
                x: 520,
                y: 240,
                width: 40,
                height: 40,
                startX: 520,
                endX: 720,
                speed: 150,
                direction: 1
            },
        ];
    }
}
